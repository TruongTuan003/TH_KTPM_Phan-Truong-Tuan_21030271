...
public class OrderController {
    ...
    public ResponseEntity<Order> createOrder(@RequestBody Order order) {
        // Kiểm tra khách hàng
        ResponseEntity<Object> customerResponse = restTemplate.getForEntity(
            "http://api-gateway:8080/customers/" + order.getCustomerId(), Object.class);
        ...
        ResponseEntity<Product> productResponse = restTemplate.getForEntity(
            "http://api-gateway:8080/products/" + order.getProductId(), Product.class);
        if (!productResponse.getStatusCode().is2xxSuccessful() || productResponse.getBody().getStock() < order.getQuantity()) {
            return ResponseEntity.badRequest().build();
        }
